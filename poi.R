...
mongo <- mongo.create("178.62.232.68")
  ...
  tweets <- mongo.find.all(mongo, coll,  query='{"_type":"twitter", "language": {"$exists": true}}',fields='{"interaction.content": 1, "language.tag":1, "classification.poi":1, "_id":0}', data.frame=TRUE)
  ...
  tweets$poi <- as.factor(tweets$poi)
  ...
  classifier = naiveBayes(matrix_train, mtweets_train$poi)
  ...
  saveRDS(classifier, paste0(path,"poiNaiveBayes.rds"),compress=F)
  
  
  accuracy <- recall_accuracy(mtweets_test$poi, predicted)
  
  #accuracy
  
  
 # num_poi <- as.numeric(mtweets_test$poi)
  
  #container = create_container(mat, as.numeric(mtweets_en$poi), trainSize=1:sample_size, testSize=(sample_size+1):num_tweets,virgin=FALSE)
  ...
  #table(num_poi, results[,"FORESTS_LABEL"])
  #table(num_poi, results[,"MAXENTROPY_LABEL"])
  
  # recall accuracy
  #recall_accuracy(num_poi, results[,"FORESTS_LABEL"])
  #recall_accuracy(num_poi, results[,"MAXENTROPY_LABEL"])
  #recall_accuracy(num_poi, results[,"TREE_LABEL"])
  #recall_accuracy(num_poi, results[,"BAGGING_LABEL"])
  #recall_accuracy(num_poi, results[,"SVM_LABEL"])
  ...
  #saveRDS(models, "poiModels.rds",compress=F)
  #saveRDS(model, "poiModel.rds",compress=F)
  #saveRDS(mat, "poiMatrix.rds",compress=F)
  #saveRDS(levels(mtweets_test$poi), "poiLabels.rds",compress=F)
